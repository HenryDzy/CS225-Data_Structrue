...
void KDTree<Dim>::KDTreeBuilder(KDTreeNode*& node, int dim, vector<Point<Dim>>& Points, int v_left, int v_right)
{
  ...
  int mid = (v_left + v_right) / 2; //the mid index
...
template <int Dim>
int KDTree<Dim>::partition(vector<Point<Dim>> to_find, int left, int right, int pivotIndex, int dim)
{
...
template <int Dim>
Point<Dim> KDTree<Dim>::quickSelect(vector<Point<Dim>>& to_sort, int left, int right, int dim, int k)
{
  if(left == right){     // If the list contains only one element,
    return to_sort[left];
  }
  int pivotIndex = (left + right) / 2;    // set pivot index
