...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    //push frequencies into singlequene
    TreeNode* push_single;
    for(long unsigned int i = 0; i < frequencies.size(); i++){
        Frequency frequency = frequencies[i];
        push_single = new TreeNode(frequency);
        singleQueue.push(push_single);
    }

    TreeNode* push_merge, *single_first, *single_second;
    //merge singlequene
    while(singleQueue.size()+mergeQueue.size()!=1) {
        single_first = removeSmallest(singleQueue, mergeQueue);
        single_second = removeSmallest(singleQueue, mergeQueue);
        push_merge = new TreeNode(single_first->freq.getFrequency() + single_second->freq.getFrequency());
        push_merge->left = single_first;
        push_merge->right = single_second;
        mergeQueue.push(push_merge);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    while(bfile.hasBits()){
        if(bfile.getNextBit()==1){
            new_leaf = new TreeNode(Frequency(bfile.getNextByte(), 0));
            return new_leaf;
        }
        else{
            new_leaf = new TreeNode(0);
            new_leaf->left = readTree(bfile);
            new_leaf->right = readTree(bfile);
            return new_leaf;
        }
    }
    return NULL;
