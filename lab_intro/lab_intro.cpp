...
PNG createSpotlight(PNG image, int centerX, int centerY) {
  int distance;
  for (unsigned x = 0; x < image.width(); x++) {
    for (unsigned y = 0; y < image.height(); y++) {   // iterate throught all pixels
      distance = sqrt(((x-centerX)*(x-centerX))+((y-centerY)*(y-centerY))); //calculate the distance
      if (distance <= 200){
        HSLAPixel *pixel = image.getPixel(x, y);
        pixel->l = (1- 0.005*distance)*pixel->l;   //calculate the luminance
      }
      else {
        HSLAPixel *pixel = image.getPixel(x, y);
        pixel->l = 0;
...
PNG watermark(PNG firstImage, PNG secondImage) {
  for (unsigned x = 0; x < secondImage.width(); x++) {
    for (unsigned y = 0; y < secondImage.height(); y++) {
      ...
      HSLAPixel *pixel_1 = firstImage.getPixel(x, y); //get two images
      if(pixel_2->l == 1){
        pixel_1->l += 0.2;      //calculate the luminance
      }
    }
  }
