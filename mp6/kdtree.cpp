...
void KDTree<Dim>::FindNNhelper(Point<Dim> &query, Point<Dim> &currBest, KDTreeNode *node, int dim) const
{
  ...
  { // Deside which region the target is in
    next = node->left;
    other = node->right;
  }
  else
  ...
  if (dist <= radius)
  { // compare if the node is in the current best radius
