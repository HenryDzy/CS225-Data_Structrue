...
class BinaryTree
{
  ...
  private:
    ...
    bool isOrdered(Node* node, int& prev) const;

    void traverse_node(Node* node, vector<vector<T> > &paths, vector<T>& path_buffer) const;
    
    int sumDistances(Node* node, int distance) const;
    /**
