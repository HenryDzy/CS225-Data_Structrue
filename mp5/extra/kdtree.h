...
class KDTree
{
  ...
  public:
    ...
    Point<Dim> quickSelect(vector<Point<Dim>>& to_sort, int left, int right, int dim, int k);

    int partition(vector<Point<Dim>> to_find, int left, int right, int pivotIndex, int dim);
