...
class KDTree
{
  ...
  public:
    ...
    Point<Dim> quickSelect(vector<Point<Dim>>& to_sort, int left, int right, int dim, size_t k);

    int partition(vector<Point<Dim>>& to_find, int left, int right, size_t pivotIndex, int dim);

    void KDTreeBuilder(KDTreeNode*& node, int dim, vector<Point<Dim>>& Points, int v_left, int v_right);

    void copy_helper(KDTreeNode*& node, KDTreeNode*& other_node);

    void destruct_helper(KDTreeNode* node);

    void FindNNhelper(Point<Dim>& query, Point<Dim>& currBest, KDTreeNode* node, int dim) const;
