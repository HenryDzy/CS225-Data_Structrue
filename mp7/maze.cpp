...
void SquareMaze::makeMaze(int width, int height){
    ...
    int i=0;
    while(i < width){   //allocate each maze pointer
...
{   
    ...
    // find which cell in the bottom row is furthest
    int maxDist = 0;
    int destX;
    for(int i = 0; i < width; i++) {
    ...
    // use parent vector to backtrack from dest 
...
PNG* SquareMaze::drawMazeWithSolution()
{
    ...
    PNG *ret = drawMaze();
    vector<int> path = solveMaze();//get the soltuion path
